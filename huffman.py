# let not this part confuse you, only this array was used from AlexMax's implementation
...
class Huffman:
	def __init__(self):
		global SKULLTAG_FREQS;
		self.build_tree(SKULLTAG_FREQS);

	def build_tree(self, freq):
		self.huff_tree = None;
		self.huff_lookup = [None] * 256;
		work = [None] * 256;
		for i in range(0, 256):
			...
			work[i].zero = None;
			work[i].one = None;
			self.huff_lookup[i] = HuffmanTab();
			self.huff_lookup[i].len = 0;
		tmp = None;
		for i in range(0, 255):
			...
			for j in range(0, 256):
				...
				if work[j].freq < min1:
					...
					minat1 = j;
					min1 = work[j].freq;
				elif work[j].freq < min2:
					minat2 = j;
					min2 = work[j].freq;
			
			if minat1 < 0:
				raise ValueError("minat1: (%f, byte %d)" % (minat1, i));
			if minat2 < 0:
				raise ValueError("minat2: (%f, byte %d)" % (minat2, i));

			tmp = HuffmanNode();
			tmp.zero = work[minat2];
			tmp.one = work[minat1];
			tmp.freq = R(work[minat2].freq+work[minat1].freq);
			tmp.val = 0xFF;
			work[minat1] = tmp;
			work[minat2] = None;
		self.huff_tree = tmp;
		self.find_tab(self.huff_tree, 0, 0);

	def find_tab(self, tmp, _len, bits):
		if tmp is None:
			raise ValueError("no huff node");
		if tmp.zero is not None:
			if tmp.one is None:
				raise ValueError("no one in node");
			if _len >= 32:
				raise ValueError("compression screwd");
			self.find_tab(tmp.zero, _len+1, ((bits << 1) & 0xFFFFFFFF));
			self.find_tab(tmp.one, _len+1, (((bits << 1) | 1) & 0xFFFFFFFF));
			return;
		self.huff_lookup[tmp.val].len = _len;
		self.huff_lookup[tmp.val].bits = bits;

	def get_bit(self, buf, pos):
		global masks;
		return ((buf[int((pos)/8)+1] & masks[int(pos%8)]) != 0);

	def put_bit(self, buf, pos, val):
		global masks;
		mask = masks[int(pos%8)];
		
		if val:
			buf[int(pos/8)+1] |= mask;
		else:
			buf[int(pos/8)+1] &= ~mask;

	def decode(self, _in):
		_in = bytearray(_in); # just to make sure
		if _in[0] == 0xFF: # not encoded
			return _in[1:];
		tbits = (len(_in)-1)*8-_in[0];
		bits = 0;
		_out = bytearray(0);
		while bits < tbits:
			tmp = self.huff_tree;
			while True: # do {
				if self.get_bit(_in, bits):
					tmp = tmp.one;
				else:
					tmp = tmp.zero;
				bits += 1;
				if tmp.zero is None: break; # } while (tmp->zero);
			_out.append(tmp.val);
		return _out;

	# expecting: '\x02\xbb\xa4\xb5\x91\xc6\xae\x95' from '\x01\x00\x00\x00test\x00'
	# output: '\x02\xbb\xa4\xb5\x91\xc6\xae\x15', same given by AlexMax's implementation
	# most likely - uninitialized output memory in Zandronum itself
	def encode(self, _in):
		_in = bytearray(_in);
		bitat = 0;
		inlen = len(_in);
		for i in range(0, inlen):
			bitat += self.huff_lookup[_in[i]].len;
		outlen = int(1+(bitat+7)/8);
		if outlen >= inlen+1:
			_out = _in[:];
			_out.insert(0, 0xFF);
			return _out;
		_out = bytearray([0] * outlen);
		bitat = 0;
		for i in range(0, inlen):
			t = self.huff_lookup[_in[i]].bits;
			for j in range(0, self.huff_lookup[_in[i]].len):
				self.put_bit(_out, bitat+self.huff_lookup[_in[i]].len-j-1, (t&1 != 0));
				t >>= 1;
			bitat += self.huff_lookup[_in[i]].len;
		_out[0] = 8*(outlen-1)-bitat;
		return _out;